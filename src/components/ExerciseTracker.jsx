...
const INTENSITIES = [
  { value: 'Light',    color: 'text-sage-500' },
  { value: 'Moderate', color: 'text-warm-500' },
...
function HeatMap({ exercises }) {
  ...
  return (
    <div>
      <p className="text-xs text-stone-400 mb-2">60 days</p>
  ...
  return (
    <div className="fade-in space-y-5">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-semibold text-stone-700">Exercise</h1>
          <p className="text-sm text-stone-400 mt-0.5">Movement is part of treatment</p>
        </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        {[
          { label: 'This week',      value: stats.thisWeek,  suffix: 'sessions' },
          { label: 'Total sessions', value: stats.total,     suffix: '' },
          { label: 'Day streak',     value: stats.streak,    suffix: 'days' },
          { label: 'Total minutes',  value: stats.totalMin,  suffix: 'min' },
        ].map(s => (
      ...
      {(exercises || []).length > 0 && (
        <div className="card overflow-x-auto">
          <h2 className="font-medium text-stone-700 mb-3 flex items-center gap-2">
            <TrendingUp size={16} className="text-sage-500" /> Activity
          </h2>
      ...
      {recent.length > 0 ? (
        <div className="space-y-2">
          <h2 className="font-medium text-stone-700">History</h2>
          {recent.map(ex => (
        ...
        <div className="card text-center py-10 border-dashed border-stone-200">
          ...
          <p className="text-stone-400 text-sm mb-3">Log your first workout</p>
          <p className="text-stone-300 text-xs">Even 15 minutes of walking counts</p>
        </div>
      )}
        ...
        <div className="modal-overlay" onClick={e => e.target === e.currentTarget && setShowModal(false)}>
          <div className="modal-box">
            <div className="flex items-center justify-between mb-4">
              <h2 className="font-semibold text-stone-700">Log a workout</h2>
